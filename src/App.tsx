...
export default function App(){
  ...
  return <>
    ...
    <main>
      <section className="hero" id="home"><div className="profileHead"><div><h1>조민수</h1><span>MIN SOO CHO</span></div><p>Frontend / Full Stack Developer</p><div className="profileLinks"><a href="mailto:mschoyb@naver.com">EMAIL</a><a href="https://github.com/msckr" target="_blank" rel="noreferrer">GITHUB</a></div></div><div className="heroCopy"><strong>사용자의 화면부터 서버의 데이터 흐름까지 이해하는 프론트엔드 개발자</strong><p>React와 TypeScript를 중심으로 실시간 거래 플랫폼과 사내 업무 시스템을 개발해 왔습니다. 기능 구현에 그치지 않고 사용자가 마주하는 화면과 그 뒤에서 움직이는 데이터의 전체 흐름을 함께 고민합니다.</p><p>React Query와 Zustand로 상태의 책임을 명확히 나누고, WebSocket 기반 실시간 데이터와 재사용 가능한 컴포넌트를 설계합니다. Java·Spring 백엔드 경험을 바탕으로 동료와 원활하게 소통하며 유지보수하기 좋은 제품을 만드는 것을 지향합니다.</p></div></section>

      <section className="intro" id="about"><div className="introGrid"><h2>문제를 구조화하고<br/><span>끝까지 해결합니다.</span></h2><div><p className="lead">요구사항의 의도를 이해하고, 복잡한 상태와 데이터 흐름을 팀이 함께 관리할 수 있는 구조로 바꿉니다.</p><p>실시간 거래 화면, 전자결재와 권한별 조회, 검색·필터·페이지네이션 등 다양한 기능을 개발했습니다. 문제가 발생하면 UI, API, 데이터베이스를 순서대로 추적해 원인을 분리하고 해결 과정을 공유합니다. 읽기 쉬운 코드와 재사용 가능한 구조로 팀의 다음 개발이 더 수월해지는 결과를 만들고자 합니다.</p><div className="career"><strong>2025.06 — NOW</strong><span>주식회사 유더블유에스<br/>기업부설연구소 · 풀스택 개발자</span></div></div></div></section>
      ...
      <section className="contact" id="contact"><p>사용자에게 가치 있는 제품을<br/>함께 만들 기회를 기다립니다.</p><a href="mailto:mschoyb@naver.com"><Mail/>mschoyb@naver.com<ArrowUpRight/></a><div className="contactFoot"><span>© 2026 MINSOO CHO</span><a href="https://github.com/msckr" target="_blank" rel="noreferrer"><Code2/>GITHUB</a></div></section>
    </main>
  </>
}
